...
- (void)layoutSubviews 
{
   ...
   CGSize contentSize = CGSizeMake(visibleWidth, (rowHeight * rowCount + spaceHeight) + 2*rowHeight);
...
#else
    CGRect extendedVisibleBounds = CGRectMake(visibleBounds.origin.x, MAX(0, visibleBounds.origin.y - 2*rowHeight), visibleBounds.size.width, visibleBounds.size.height + 4*rowHeight);
#endif
  ...
  // changed by tanyu. 2012.05.12. 
  // 为了使快速滚动时没不易看到图片异步加载
#if 0   
    NSInteger stopAtIndex = MIN(thumbCount, (bottomRow * itemsPerRow) + itemsPerRow);
#else
    NSInteger stopAtIndex = MIN(thumbCount, (bottomRow * itemsPerRow) + 3*itemsPerRow);
#endif
