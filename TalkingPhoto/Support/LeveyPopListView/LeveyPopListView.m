...
#define POPLISTVIEW_SCREENINSET_VERTICAL 80
...
#else
      _tableView = [[UITableView alloc] initWithFrame:CGRectMake(POPLISTVIEW_SCREENINSET, 
                                                                 POPLISTVIEW_SCREENINSET_VERTICAL + POPLISTVIEW_HEADER_HEIGHT, 
                                                                 rect.size.width - 2 * POPLISTVIEW_SCREENINSET,
                                                                 rect.size.height - 2 * POPLISTVIEW_SCREENINSET_VERTICAL - POPLISTVIEW_HEADER_HEIGHT - RADIUS)];
#endif
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  // added by tanyu. tag与相册goup各类相同 2012.05.12
    cell.tag = [[[_options objectAtIndex:row] objectForKey:@"tag"] intValue];
  // end
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(leveyPopListView:didSelectedIndex:)]) {
        [self.delegate leveyPopListView:self didSelectedIndex:indexPath];
    }
...
#else
  CGRect bgRect = CGRectInset(rect, POPLISTVIEW_SCREENINSET, POPLISTVIEW_SCREENINSET_VERTICAL);
  CGRect titleRect = CGRectMake(POPLISTVIEW_SCREENINSET + 10, POPLISTVIEW_SCREENINSET_VERTICAL + 10 + 5,
                                rect.size.width -  2 * (POPLISTVIEW_SCREENINSET_VERTICAL + 10), 30);
  CGRect separatorRect = CGRectMake(POPLISTVIEW_SCREENINSET, POPLISTVIEW_SCREENINSET_VERTICAL + POPLISTVIEW_HEADER_HEIGHT - 3,
                                    rect.size.width - 2 * POPLISTVIEW_SCREENINSET, 2);
...
#else
  float y = POPLISTVIEW_SCREENINSET_VERTICAL;
#endif
